...
func makeFakeTopologyRelationships(cmps []topologyComponentObservedPayload) []topologyRelationshipObservedPayload {
	...
	return []topologyRelationshipObservedPayload{
		...
		rel("public_api_gateway", "calls", "auth_service", "Public API Gateway calls Auth Listener"),
		rel("public_api_gateway", "calls", "catalog_service", "Public API Gateway calls Catalog Listener"),
		rel("public_api_gateway", "calls", "search_api", "Public API Gateway calls Search API"),
		rel("public_api_gateway", "calls", "checkout_service", "Public API Gateway calls Checkout Listener"),
		rel("checkout_service", "calls", "auth_service", "Checkout Listener calls Auth Listener"),
		rel("checkout_service", "calls", "search_api", "Checkout Listener calls Search API"),
		rel("checkout_service", "calls", "inventory_service", "Checkout Listener calls Inventory Listener"),
		rel("checkout_service", "calls", "payments_service", "Checkout Listener calls Payments Listener"),
		rel("checkout_service", "calls", "orders_service", "Checkout Listener calls Orders Listener"),
		rel("orders_service", "calls", "notifications_service", "Orders Listener calls Notifications Listener"),
		rel("auth_service", "reads_from", "redis_sessions", "Auth Listener reads from Redis Sessions"),
		rel("auth_service", "writes_to", "users_postgres", "Auth Listener writes to Users Postgres"),
		rel("catalog_service", "writes_to", "catalog_postgres", "Catalog Listener writes to Catalog Postgres"),
		rel("catalog_service", "publishes_to", "order_events_queue", "Catalog Listener publishes to Order Events Queue"),
		...
		rel("orders_service", "writes_to", "orders_postgres", "Orders Listener writes to Orders Postgres"),
		rel("orders_service", "publishes_to", "order_events_queue", "Orders Listener publishes to Order Events Queue"),
		rel("payments_service", "writes_to", "payments_postgres", "Payments Listener writes to Payments Postgres"),
		rel("payments_service", "calls", "stripe", "Payments Listener calls Stripe"),
		rel("inventory_service", "reads_from", "catalog_postgres", "Inventory Listener reads from Catalog Postgres"),
		...
		rel("notifications_service", "publishes_to", "order_events_queue", "Notifications Listener publishes to Order Events Queue"),
		rel("notifications_service", "calls", "sendgrid", "Notifications Listener calls SendGrid"),
		rel("orders_service", "writes_to", "s3_invoice_bucket", "Orders Listener writes to Invoice S3 Bucket"),
		rel("identity_team", "owns", "auth_service", "Identity Team owns Auth Listener"),
		rel("identity_team", "owns", "users_postgres", "Identity Team owns Users Postgres"),
		rel("commerce_team", "owns", "catalog_service", "Commerce Team owns Catalog Listener"),
		rel("commerce_team", "owns", "search_api", "Commerce Team owns Search API"),
		rel("commerce_team", "owns", "checkout_service", "Commerce Team owns Checkout Listener"),
		rel("commerce_team", "owns", "orders_service", "Commerce Team owns Orders Listener"),
		rel("commerce_team", "owns", "payments_service", "Commerce Team owns Payments Listener"),
		rel("platform_team", "owns", "public_api_gateway", "Platform Team owns Public API Gateway"),
		rel("platform_team", "owns", "notifications_service", "Platform Team owns Notifications Listener"),
		rel("auth_service", "processes_entity", "customer", "Auth Listener processes Customer"),
		rel("catalog_service", "processes_entity", "product", "Catalog Listener processes Product"),
		rel("checkout_service", "processes_entity", "cart", "Checkout Listener processes Cart"),
		rel("orders_service", "processes_entity", "order", "Orders Listener processes Order"),
		rel("payments_service", "processes_entity", "payment", "Payments Listener processes Payment"),
		rel("orders_service", "processes_entity", "invoice", "Orders Listener processes Invoice"),
