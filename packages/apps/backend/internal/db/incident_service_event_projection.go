...
func (s *IncidentService) HandleEventProjection(ctx context.Context, event *ent.NormalizedEvent) ([]rez.ProjectedEntityRef, error) {
...
func (s *IncidentService) handleIncidentEventProjection(ctx context.Context, ie *projections.IncidentEvent) ([]rez.ProjectedEntityRef, error) {
	...
	return projEnts, s.db.WithTx(ctx, func(ctx context.Context, client *ent.Client) error {
		kneId, evidenceErr := s.knowledge.IngestDomainEntityEvidence(ctx, ie.Event, incidentObservedEvidence)
		if evidenceErr != nil {
		...
		projEnts = append(projEnts, rez.ProjectedEntityRef{
			Kind: "incident",
			Id:   inc.ID,
		})
