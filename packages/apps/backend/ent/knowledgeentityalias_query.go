...
func (_q *KnowledgeEntityAliasQuery) QueryEvidence() *KnowledgeEvidenceQuery {
...
func (_q *KnowledgeEntityAliasQuery) WithEvidence(opts ...func(*KnowledgeEvidenceQuery)) *KnowledgeEntityAliasQuery {
	query := (&KnowledgeEvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *KnowledgeEntityAliasQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*KnowledgeEntityAlias, error) {
	...
	if query := _q.withEvidence; query != nil {
		if err := _q.loadEvidence(ctx, query, nodes,
			func(n *KnowledgeEntityAlias) { n.Edges.Evidence = []*KnowledgeEvidence{} },
			func(n *KnowledgeEntityAlias, e *KnowledgeEvidence) { n.Edges.Evidence = append(n.Edges.Evidence, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *KnowledgeEntityAliasQuery) loadEvidence(ctx context.Context, query *KnowledgeEvidenceQuery, nodes []*KnowledgeEntityAlias, init func(*KnowledgeEntityAlias), assign func(*KnowledgeEntityAlias, *KnowledgeEvidence)) error {
	...
	query.Where(predicate.KnowledgeEvidence(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(knowledgeentityalias.EvidenceColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
