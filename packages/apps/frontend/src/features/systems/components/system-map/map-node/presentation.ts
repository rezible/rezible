import { getMapCategoryDisplay, isActorCategory } from "$features/systems/lib/system-map/category";
...
export const nodePresentationForEntity = ({category, kind, label}: GraphEntity): MapNodePresentation => {
	const isActor = isActorCategory(category);
	...
	return { label, categoryLabel, kindLabel, isActor };
};
