...
>
				...
				<div class="grid gap-2 sm:grid-cols-2">
					{#each controller.tags as tag}
						<label class="border-border flex items-center gap-2 border px-2 py-2 text-xs">
							<Checkbox
								bind:checked={
									() => controller.hasTag(tag.id), () => controller.toggleTag(tag.id)
								}
					...
					{#each controller.fields as field}
						<div class="space-y-2">
							...
							<Select.Root
								...
								bind:value={
									() => controller.getFieldSelection(field.id),
									(value) => controller.setFieldSelection(field.id, value)
								}
							>
	...
	<div class="border-t px-4 py-3">
		<Dialog.Footer>
			<Button type="button" variant="outline" onclick={() => controller.setOpen(false)}>Cancel</Button>
