<script lang="ts">
	...
	const systemAnalysisController = initSystemAnalysisController(() => viewController.systemAnalysisId || "");
...
<div class="flex h-full min-h-0 min-w-0 flex-col gap-3 overflow-hidden">
	...
	{:else if retroQuery.error && !viewController.retrospective}
		<div role="alert" class="p-4">
			...
			<Button
				variant="outline"
				onclick={() => retroQuery.refetch()}
			>Retry analysis</Button>
		</div>
	{:else if !systemAnalysisController.analysisId}
	...
	{:else}
		<div class="flex shrink-0 flex-wrap items-center gap-2">
			<Button 
				variant="outline" 
				size="sm" 
				onclick={systemAnalysisController.refreshAll}
		...
		<div class="min-h-0 flex-1">
			<SystemAnalysisDiagram interaction={controller.graphInteraction}>
				{#snippet inspector()}
					<AnalysisInspector />
				{/snippet}
