...
export class UserViewController {
	...
	userLocalTime = $derived(
		new Date().toLocaleTimeString([], { timeZone: this.timeZone, hour: "2-digit", minute: "2-digit" })
	);
...
const ctx = new Context<UserViewController>("UserViewController");
export const initUserViewController = (idFn: Getter<string>) => ctx.set(new UserViewController(idFn));
export const useUserViewController = () => ctx.get();
