<script lang="ts">
	...
	const attrs = $derived(view.alert?.attributes);
...
<div class="flex gap-2">
	<div class="flex flex-col gap-2 min-w-96 max-w-xl">
		<div class="flex flex-col gap-2 border p-2 h-fit">
			...
			{#if !!attrs && !attrs?.description}
				<span class="text-foreground/60">No Description Provided</span>
			{:else}
		...
		{#if !!attrs?.definition}
			<div class="flex flex-col gap-2 border p-2 h-fit">
				<span class="uppercase font-semibold text-foreground/90">Definition</span>
		...
		{#if !!roster}
			<div class="flex flex-col gap-2 border p-2 h-fit">
				<span class="uppercase font-semibold text-foreground/90">Roster</span>
	...
	<div class="flex flex-col gap-2 border p-2">
		<span class="uppercase font-semibold text-foreground/90">Metrics</span>
