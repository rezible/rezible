...
func (sq *SubscriptionQuery) QueryTeam() *TeamQuery {
...
func (sq *SubscriptionQuery) QueryIncident() *IncidentQuery {
	query := (&IncidentClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) Clone() *SubscriptionQuery {
	...
	return &SubscriptionQuery{
		...
		withIncident: sq.withIncident.Clone(),
...
func (sq *SubscriptionQuery) WithTeam(opts ...func(*TeamQuery)) *SubscriptionQuery {
	query := (&TeamClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SubscriptionQuery) WithIncident(opts ...func(*IncidentQuery)) *SubscriptionQuery {
	query := (&IncidentClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withIncident = query
	return sq
}
...
//		GroupBy(subscription.FieldEvent).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Subscription.Query().
//		Select(subscription.FieldEvent).
//		Scan(ctx, &v)
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if sq.withUser != nil || sq.withTeam != nil || sq.withIncident != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withIncident; query != nil {
		if err := sq.loadIncident(ctx, query, nodes, nil,
			func(n *Subscription, e *Incident) { n.Edges.Incident = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscriptionQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subscription_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SubscriptionQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Team)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subscription)
	for i := range nodes {
		if nodes[i].subscription_team == nil {
			continue
		}
		fk := *nodes[i].subscription_team
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subscription_team" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SubscriptionQuery) loadIncident(ctx context.Context, query *IncidentQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Incident)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subscription)
	for i := range nodes {
		if nodes[i].subscription_incident == nil {
			continue
		}
		fk := *nodes[i].subscription_incident
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subscription_incident" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
