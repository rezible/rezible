...
// The AlertMetricsMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type AlertMetricsMutationRuleFunc func(context.Context, *ent.AlertMetricsMutation) error

// EvalMutation calls f(ctx, m).
func (f AlertMetricsMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.AlertMetricsMutation); ok {
		return f(ctx, m)
	}
