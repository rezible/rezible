...
func (eq *EnvironmentQuery) QueryIncidents() *IncidentQuery {
	query := (&IncidentClient{config: eq.config}).Query()
...
func (eq *EnvironmentQuery) Clone() *EnvironmentQuery {
	...
	return &EnvironmentQuery{
		...
		withIncidents: eq.withIncidents.Clone(),
...
func (eq *EnvironmentQuery) WithIncidents(opts ...func(*IncidentQuery)) *EnvironmentQuery {
	query := (&IncidentClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withIncidents = query
	return eq
}
...
//		GroupBy(environment.FieldArchiveTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Environment.Query().
//		Select(environment.FieldArchiveTime).
//		Scan(ctx, &v)
...
func (eq *EnvironmentQuery) prepareQuery(ctx context.Context) error {
	...
	if err := environment.Policy.EvalQuery(ctx, eq); err != nil {
		return err
	}
	return nil
}
...
func (eq *EnvironmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Environment, error) {
	...
	if query := eq.withIncidents; query != nil {
		if err := eq.loadIncidents(ctx, query, nodes,
			func(n *Environment) { n.Edges.Incidents = []*Incident{} },
			func(n *Environment, e *Incident) { n.Edges.Incidents = append(n.Edges.Incidents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EnvironmentQuery) loadIncidents(ctx context.Context, query *IncidentQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *Incident)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(environment.IncidentsTable)
		s.Join(joinT).On(s.C(incident.FieldID), joinT.C(environment.IncidentsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(environment.IncidentsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(environment.IncidentsPrimaryKey[1]))
