...
func (sq *ServiceQuery) QueryIncidents() *IncidentResourceImpactQuery {
	query := (&IncidentResourceImpactClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) QueryOwnerTeam() *TeamQuery {
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withIncidents: sq.withIncidents.Clone(),
		withOwnerTeam: sq.withOwnerTeam.Clone(),
...
func (sq *ServiceQuery) WithIncidents(opts ...func(*IncidentResourceImpactQuery)) *ServiceQuery {
	query := (&IncidentResourceImpactClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withIncidents = query
	return sq
}
...
func (sq *ServiceQuery) WithOwnerTeam(opts ...func(*TeamQuery)) *ServiceQuery {
	query := (&TeamClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withOwnerTeam = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [2]bool{
			sq.withIncidents != nil,
			sq.withOwnerTeam != nil,
		}
	)
	if sq.withOwnerTeam != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withIncidents; query != nil {
		if err := sq.loadIncidents(ctx, query, nodes,
			func(n *Service) { n.Edges.Incidents = []*IncidentResourceImpact{} },
			func(n *Service, e *IncidentResourceImpact) { n.Edges.Incidents = append(n.Edges.Incidents, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withOwnerTeam; query != nil {
		if err := sq.loadOwnerTeam(ctx, query, nodes, nil,
...
func (sq *ServiceQuery) loadIncidents(ctx context.Context, query *IncidentResourceImpactQuery, nodes []*Service, init func(*Service), assign func(*Service, *IncidentResourceImpact)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(incidentresourceimpact.FieldServiceID)
	}
...
func (sq *ServiceQuery) loadOwnerTeam(ctx context.Context, query *TeamQuery, nodes []*Service, init func(*Service), assign func(*Service, *Team)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Service)
	for i := range nodes {
		if nodes[i].service_owner_team == nil {
			continue
		}
		fk := *nodes[i].service_owner_team
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_owner_team" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
