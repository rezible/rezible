...
type EventQuery struct {
	...
	withIncidentEvent *IncidentEventQuery
	withAnnotations   *EventAnnotationQuery
...
func (eq *EventQuery) QueryTenant() *TenantQuery {
	query := (&TenantClient{config: eq.config}).Query()
...
func (eq *EventQuery) QueryAlertInstance() *AlertInstanceQuery {
	query := (&AlertInstanceClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryIncidentEvent chains the current query on the "incident_event" edge.
func (eq *EventQuery) QueryIncidentEvent() *IncidentEventQuery {
	query := (&IncidentEventClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) QueryAnnotations() *EventAnnotationQuery {
	query := (&EventAnnotationClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withTenant:        eq.withTenant.Clone(),
		withAlertInstance: eq.withAlertInstance.Clone(),
		withIncidentEvent: eq.withIncidentEvent.Clone(),
		withAnnotations:   eq.withAnnotations.Clone(),
...
func (eq *EventQuery) WithTenant(opts ...func(*TenantQuery)) *EventQuery {
	query := (&TenantClient{config: eq.config}).Query()
...
func (eq *EventQuery) WithAlertInstance(opts ...func(*AlertInstanceQuery)) *EventQuery {
	query := (&AlertInstanceClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withAlertInstance = query
	return eq
}

// WithIncidentEvent tells the query-builder to eager-load the nodes that are connected to
// the "incident_event" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithIncidentEvent(opts ...func(*IncidentEventQuery)) *EventQuery {
	query := (&IncidentEventClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withIncidentEvent = query
	return eq
}
...
func (eq *EventQuery) WithAnnotations(opts ...func(*EventAnnotationQuery)) *EventQuery {
	query := (&EventAnnotationClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withAnnotations = query
	return eq
}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	var (
		...
		loadedTypes = [4]bool{
			eq.withTenant != nil,
			eq.withAlertInstance != nil,
			eq.withIncidentEvent != nil,
			eq.withAnnotations != nil,
		}
	)
	...
	if query := eq.withTenant; query != nil {
		if err := eq.loadTenant(ctx, query, nodes, nil,
			func(n *Event, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withAlertInstance; query != nil {
		if err := eq.loadAlertInstance(ctx, query, nodes,
			func(n *Event) { n.Edges.AlertInstance = []*AlertInstance{} },
			func(n *Event, e *AlertInstance) { n.Edges.AlertInstance = append(n.Edges.AlertInstance, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withIncidentEvent; query != nil {
		if err := eq.loadIncidentEvent(ctx, query, nodes,
			func(n *Event) { n.Edges.IncidentEvent = []*IncidentEvent{} },
			func(n *Event, e *IncidentEvent) { n.Edges.IncidentEvent = append(n.Edges.IncidentEvent, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withAnnotations; query != nil {
		if err := eq.loadAnnotations(ctx, query, nodes,
			func(n *Event) { n.Edges.Annotations = []*EventAnnotation{} },
			func(n *Event, e *EventAnnotation) { n.Edges.Annotations = append(n.Edges.Annotations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Event, init func(*Event), assign func(*Event, *Tenant)) error {
...
func (eq *EventQuery) loadAlertInstance(ctx context.Context, query *AlertInstanceQuery, nodes []*Event, init func(*Event), assign func(*Event, *AlertInstance)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(alertinstance.FieldEventID)
	}
	query.Where(predicate.AlertInstance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.AlertInstanceColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (eq *EventQuery) loadIncidentEvent(ctx context.Context, query *IncidentEventQuery, nodes []*Event, init func(*Event), assign func(*Event, *IncidentEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Event)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(incidentevent.FieldEventID)
	}
	query.Where(predicate.IncidentEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.IncidentEventColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (eq *EventQuery) loadAnnotations(ctx context.Context, query *EventAnnotationQuery, nodes []*Event, init func(*Event), assign func(*Event, *EventAnnotation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Event)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(eventannotation.FieldEventID)
	}
	query.Where(predicate.EventAnnotation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.AnnotationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (eq *EventQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withTenant != nil {
			_spec.Node.AddColumnOnce(event.FieldTenantID)
		}
	}
