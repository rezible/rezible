...
CREATE SCHEMA IF NOT EXISTS river AUTHORIZATION rez_migrator;
...
ALTER DEFAULT PRIVILEGES
    FOR USER rez_migrator IN SCHEMA rezible
    GRANT SELECT, INSERT, UPDATE, DELETE
    ON TABLES TO rez_app;
ALTER DEFAULT PRIVILEGES
    FOR USER rez_migrator IN SCHEMA rezible
    GRANT USAGE, SELECT
    ON SEQUENCES TO rez_app;

-- Default privileges: rez_migrator-created objects in river schema
ALTER DEFAULT PRIVILEGES
    FOR USER rez_migrator IN SCHEMA river
    GRANT SELECT, INSERT, UPDATE, DELETE
    ON TABLES TO rez_app;
ALTER DEFAULT PRIVILEGES
    FOR USER rez_migrator IN SCHEMA river
...
ALTER ROLE rez_migrator SET search_path TO rezible;
