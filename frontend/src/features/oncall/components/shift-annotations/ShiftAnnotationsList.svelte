<script lang="ts">
	import { listOncallShiftAnnotationsOptions, updateOncallShiftAnnotationMutation, type OncallShiftAnnotation, type UpdateOncallShiftAnnotationRequestBody } from '$lib/api';
...
<div class="h-10 flex w-full gap-4 items-center px-2">
	<Header title="Shift Event Annotations" classes={{root:"w-full", container: "flex-1"}}>
		<div slot="actions" class:hidden={!editable}>
			<Button color="primary" variant="fill" on:click={() => {showEditorDialog = true}}>
...
{#if editable}
	<ShiftAnnotationEditorDialog {shiftId} {annotatedEventIds} bind:open={showEditorDialog} onCreated={onAnnotationCreated} />
{/if}
...
{#snippet annotationListItem(ann: OncallShiftAnnotation)}
	...
	<div class="grid grid-cols-[100px_auto_minmax(0,1fr)] place-items-center border p-2">
		...
		<div class="items-center static z-10">
			<Icon data={eventKindIcons[ann.attributes.kind]} classes={{root: "bg-accent-900 rounded-full p-2 w-auto h-10"}} />
		</div>
		...
		<div class="w-full justify-self-start grid grid-cols-[auto_40px] items-center px-2">
			...
			<div class="place-self-end flex flex-row gap-2" class:hidden={!editable}>	
				<Button 
					...
					loading={updateAnnotationMut.isPending && updateAnnotationMut.variables?.path.id === ann.id}
					icon={ann.attributes.pinned ? mdiPin : mdiPinOutline}
					color={ann.attributes.pinned ? "accent" : "default"}
					iconOnly 
					on:click={() => {togglePinned(ann)}} />		
