...
import type { OncallShiftHandover, OncallShiftHandoverSection, OncallShiftHandoverTemplate, OncallShiftHandoverTemplateSection } from '$lib/api';
...
const createHandoverState = () => {
	...
	const restoreExisting = (handover: OncallShiftHandover) => {
		...
		handover.attributes.content.forEach((sec, idx) => {
			if (sec.kind === "regular") {
				const { editor, activeStatus } = createEditor(idx, !!sec.jsonContent ? JSON.parse(sec.jsonContent) : undefined);
				sections.push({
					header: sec.header,
					kind: sec.kind,
					editor: editor,
