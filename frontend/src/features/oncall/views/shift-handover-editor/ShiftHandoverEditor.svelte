<script lang="ts">
	...
	import { getNextOncallShiftOptions, getOncallShiftHandoverOptions, getOncallShiftHandoverTemplateOptions, sendOncallShiftHandoverMutation, type OncallShift, type OncallShiftHandover, type OncallShiftHandoverTemplate, type SendOncallShiftHandoverAttributes } from '$lib/api';
	...
	const templateQuery = createQuery(() => getOncallShiftHandoverTemplateOptions({path: {id: templateId}}));
	...
	const submitHandover = () => {
		...
		console.log(content);
		sendMutation.mutate({path: {id: shiftId}, body: {attributes: {content}}});
	};
...
<div class="px-3 flex-1 flex flex-col gap-2 max-h-full min-h-0 overflow-y-auto">
	...
	<div class="grid grid-cols-5 gap-2 flex-1 min-h-0 max-h-full">
		<div class="col-span-2 flex flex-col gap-2 min-h-0 max-h-full overflow-y-auto border rounded-lg p-3 bg-surface-200">
		...
		<div class="col-span-2 flex flex-col gap-2 min-h-0 border rounded-lg p-2 h-full max-h-full overflow-hidden">
