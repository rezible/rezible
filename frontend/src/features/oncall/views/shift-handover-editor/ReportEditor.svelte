<script lang="ts">
    ...
    import { listOncallShiftAnnotationsOptions, listOncallShiftIncidentsOptions, type OncallShift, type OncallShiftHandover, type OncallShiftHandoverTemplate } from '$lib/api';
	...
	const pinnedAnnotations = $derived(annotationsQuery.data?.data.filter(ann => ann.attributes.pinned) ?? []);

	const incidentsEnabled = $derived(handoverState.sections.some(s => s.kind === "incidents"));
	const incidentsQuery = createQuery(() => ({...listOncallShiftIncidentsOptions({path: {id: shift.id}}), enabled: incidentsEnabled}));
...
{#each handoverState.sections as section, i}
	<div class="flex flex-col">
		...
		{#if section.kind === "annotations"}
			<div class="bg-surface-100 p-2">
				{#if pinnedAnnotations.length === 0}
					<span>No Events <span class="text-surface-content/50">(Pinned Annotations will be included here)</span></span>
				{:else}
					<ul class="list-disc pl-5">
						{#each pinnedAnnotations as ann (ann.id)}
							<li>{ann.attributes.title}</li>
							<ul class="pl-5">
								<li><span class="italic">{ann.attributes.notes}</span></li>
							</ul>
						{/each}
					</ul>
				{/if}
				...
				{:else}
					<ul class="list-disc pl-5">
						{#each incidents as inc (inc.id)}
							<li><a class="link" href="/incidents/{inc.id}" target="_blank">{inc.attributes.title}</a></li>
						{/each}
					</ul>
				{/if}
			</div>
		...
		{#if section.kind === "regular"}
			...
			<div
				class="bg-surface-100 p-1 h-fit" class:cursor-text={!handoverState.sent}
				onfocusin={(e) => {onSectionFocus(e, i, true)}}
				onfocusout={(e) => {onSectionFocus(e, i, false)}}
				tabindex="-1"
				spellcheck="false"
			>
				<div class="flex flex-row w-full items-center gap-2 h-fit" class:hidden={handoverState.sent} data-menu={i}>
					...
					<Button icon={mdiFormatBold} rounded={false} size="sm"
						disabled={!isActive}
						variant={(isActive && activeStatus.get("bold")) ? 'fill' : 'fill-light'}
						on:click={runEditorCmd(c => c.toggleBold())}
					/>

					<Button icon={mdiFormatListBulleted} rounded={false} size="sm"
						disabled={!isActive}
