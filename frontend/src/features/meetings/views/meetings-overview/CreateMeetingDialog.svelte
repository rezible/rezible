...
<Dialog bind:open loading={isPending} persistent on:close={resetMeetingState} classes={{root: "py-2", dialog: "max-h-full min-h-0 flex flex-col"}}>
	...
	<div class="flex flex-row gap-2 overflow-y-auto p-2 w-fit">
		<div class="w-fit flex flex-col gap-1 max-h-full">
			...
			<DateTimePickerField
				...
				onChange={newStart => {formData.start = newStart}}
			/>
...
{#snippet repeatingFields()}
	...
	{#if formData.repeats === "weekly" || formData.repeats === "monthly"}
		<Field label="Every" classes={{input: "gap-2"}} dense>
			<NumberStepper min={1} max={formData.repeats === "weekly" ? 4 : 6} bind:value={formData.repetition_step} />
