<script lang="ts">
	...
	import { API_BASE_PATH, API_AUTH_PATH } from "$lib/config";
	...
	const providers = $derived(config?.providers || []);
...
<div class="grid h-full w-full place-items-center">
	<div class="flex flex-col gap-2 border rounded-lg border-surface-content/10 bg-surface-200 p-3">
		...
		{#if errorCategory === "invalid_user"}
			<Button href="{API_AUTH_PATH}/logout" disabled={configQuery.isLoading} color="primary">Logout</Button>
		{:else}
			{#each providers as p}
				{@const display = providerDisplay.get(p.name.toLowerCase())}
				{@const startFlowPath = `${API_BASE_PATH}${p.startFlowPath}`}
				<Button href={startFlowPath} color="primary">
					<span class="flex items-center gap-2">
					Continue with {display?.label ?? p.name} ({startFlowPath})
					{#if display?.icon}
			...
			{:else}
				<span>no configured auth providers</span>
			{/each}
		{/if}
	</div>
</div>