...
<div class="flex flex-row min-h-0 max-h-full h-full gap-2 p-2">
	...
	<div class="flex flex-col gap-2 overflow-y-auto flex-1">
		...
		{#if eventType === "decision"}
			{@render panel("Decision Context", "Document the options, constraints, and reasoning behind this choice", mdiGraphOutline, DecisionContextPanel)}
		{/if}

		{@render panel("Contributing Factors", "Identify pressures and conditions that shaped this event", mdiLayers, ContributingFactorsPanel)}

		{@render panel("Evidence & Links", "Add links to logs, metrics, discussions, and other supporting information", mdiLink, EvidencePanel)}

		{@render panel("System Context", "Document the relevant system components and their conditions at this time", mdiStateMachine, SystemComponentsPanel)}
	</div>
</div>
