...
<Dialog
	open={analysis.relationshipDialogOpen}
	on:close={() => {analysis.setRelationshipDialogOpen(false)}}
	persistent
	portal
	...
	<svelte:fragment slot="default">
		{#if analysis.relationshipDialogOpen}
			<span>relationship: {analysis.editingRelationship?.id}</span>
		{/if}
	</svelte:fragment>

	<svelte:fragment slot="actions">
		<ConfirmButtons
			onClose={() => {analysis.setRelationshipDialogOpen(false)}}
