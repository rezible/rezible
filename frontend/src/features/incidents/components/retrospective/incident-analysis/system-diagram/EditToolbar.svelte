<script lang="ts">
	...
	const component = $derived(
		node?.data
			? (node.data.component as SystemAnalysisComponent)
			: undefined
	);
	const relationship = $derived(
		edge?.data
			? (edge.data.relationship as SystemAnalysisRelationship)
			: undefined
	);
		...
		>
			<ButtonGroup variant="fill-light" color="accent" size="sm">
				{#if node}
					<Button
						on:click={() =>
							analysis.setComponentDialogOpen(true, component)}
						>Edit Component</Button
					>
				{:else if edge}
					<Button
						on:click={() =>
							analysis.setRelationshipDialogOpen(
								true,
								relationship
